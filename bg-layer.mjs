  ...
  (async () => {
    ...
    async function drawMap(layer, dataset) {
      ...
      layerView = await view.whenLayerView(layer);
      // layerViewBackground = await view.whenLayerView(layerBackground);
      layerViewBackground = null;

      watchUtils.watch(layerView, 'updating', updating => console.log('layerView updating', updating));
      ...
      Object.assign(window, { view, map, dataset, layer, layerView, layerViewBackground, getDatasetField, getDatasetFieldUniqueValues, /*histogram, histogramValues,*/ generateHistogram, HistogramRangeSlider, uniqueValues });
    ...
    async function ensureBackgroundLayer () {
      if (!layerBackground) {
        ...
        layerBackground = new FeatureLayer({
        ...
        view.map.add(layerBackground);
        view.map.reorder(layerBackground, 0); // move foreground data back on top
        await view.whenLayerView(layerBackground)
      }
    ...
    async function datasetFieldIsInteger (dataset, fieldName, layer) {
      ...
      if (field.type.toLowerCase().includes('integer')) { // explicit integer type
        ...
        return stats.values.every(v => v.value == null || Number.isInteger(Number(v.value)));
      }
   }
    ...
    async function updateLayerViewEffect({
      ...
      if (where) {
        ...
        if (where !== '1=1') {
          ...
          layerViewBackground.effect = {
            filter: {
              where,
              // where: `NOT (${where})`
            },
            // excludedEffect: 'grayscale(100%) brightness(25%) opacity(10%)',
        ...
        // layerView.queryFeatureCount({ // only qeuery data in view
        layer.queryFeatureCount({ // query ALL data
          where: where || '1=1',
      ...
      if (updateExtent) {
        try {
          ...
          const queriedExtent = await layerView.layer.queryExtent({
            ...
            where: layerView.filter?.where ? concatWheres({ server: true }) : '1=1',
            outSpatialReference: layerView.view.spatialReference
          });
